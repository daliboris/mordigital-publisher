...
declare function lapi:dictionaries($request as map(*)) {  
    ...
    let $entry-parts := $request?parameters?entry-parts
    ...
    then
       ...
       let $hitsAll :=
        ...
        let $store := (
            session:set-attribute($config:session-prefix || ".hits", $hitsAll),
            session:set-attribute($config:session-prefix || ".hitCount", $hitCount),
            session:set-attribute($config:session-prefix || ".exist-db-query", $exist-db-query),
            session:set-attribute($config:session-prefix || ".search", if (empty($request?parameters?query))
             then () else xmldb:decode($request?parameters?query)),
...
(:
        let $hits := if($entry-parts = 'headword') then
            for $item in $hits//tei:form return <entry xmlns="http://www.tei-c.org/ns/1.0">{$item}</entry>
            else $hits
:)
        return if($request?parameters?format = "xml") then
...
declare %private function lapi:show-hits-xml($request as map(*), $hits as item()*, $docs as xs:string*) {
    lapi:show-hits-xml($request, $hits, $docs, (), ())
};

declare %private function lapi:show-hits-xml($request as map(*), $hits as item()*, 
        $docs as xs:string*, $containter as xs:string, $namespace as xs:string) {
    ...
    response:set-header("Content-Type", "application/xml" ),
    let $config := ()
    return if(empty($containter)) then
        subsequence($hits, $request?parameters?start, $request?parameters?per-page)
    else
        element {QName($namespace, $containter)} {subsequence($hits, $request?parameters?start, $request?parameters?per-page)}
};
...
declare function lapi:get-html($hits as item()*, $request as map(*), $config) {
    ...
    let $out := $pm-config:web-transform($xml, $params, $config?odd)
...
declare function lapi:expand($data as node()) {
    let $exist-db-query := session:get-attribute($config:session-prefix || ".exist-db-query")
...
declare %private function lapi:query-default-view($context as element()*, $exist-db-query as element(exist-db-query)) {
    ""
};
