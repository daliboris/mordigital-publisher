...
declare function edq:parse-exist-db-query($input as element(exist-db-query), $format as xs:string?) {
    let $query :=  edq:parse-query($input/query-option)
    let $options := edq:parse-options($input/query-option)
    let $fields := edq:parse-fields($input)
    let $facets := edq:parse-facets($input/facets)
    let $full-options := map:entry("full-options", map:merge(($options, $fields, $facets)))

    return if($format = "xml") then
        edq:return-as-xml($input, $query,$options,$fields, $facets, $full-options)
    else
        edq:return-as-maps($input, $query, $options, $fields, $facets)
};
...
declare %private function edq:return-as-maps($input as element(exist-db-query), $query, $options, $fields, $facets) {
    map:merge( (
            ...
            map:entry("xml", edq:return-as-xml($input, $query, $options, $fields, $facets, map:merge(($options, $fields, $facets))) )
    ) )
};

declare %private function edq:return-as-xml($input as element(exist-db-query), $query, $options, $fields, $facets, $full-options) {
<exist-db-query>
    {
          <query>{$query}</query>
        ...
        , <full-options>{edq:map-to-xml($full-options)}</full-options>
        , <input>{$input}</input>
    }
</exist-db-query>
};
